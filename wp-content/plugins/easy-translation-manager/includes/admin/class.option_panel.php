...
You need a API key to use the service. <a target="_blank" href="http://api.yandex.com/translate/">Click to get API Key</a>','etm').'', 
        ...
        //------------------------- Plugins -------------------
            ...
            $t[$i] = (object)array();
            $t[$i]->id 			= 'troubleshoot'; 
            $t[$i]->label 		= __('Troubleshooting','etm');//title on tab
            $t[$i]->right_label	= __('Fix small problems','etm');//title on tab
            $t[$i]->page_title	= __('Fix small problems','etm');//title on content
            $t[$i]->theme_option = true;
            $t[$i]->plugin_option = true;
            $temp = array();
			...
			$temp[] = (object)array(
					'id'		=> 'dont_save_overwrite',
					'label'		=> __('Dont save permalink lang.','etm'),
					'type'		=> 'onoff',
					'default'	=> false,
					'description'=> __('By default it save the languish you are on in overwrite by permalink, but if you want it to return when not on permalink then activate this function','etm'),
	                'save_option'=>true,
					'load_option'=>true
			);				
			
			$temp[] = (object)array(
					'id'		=> 'trobelshoot_use_sesstion_data',
					'label'		=> __('Use Session Data instead of wp_cache to save language info.','etm'),
					'type'		=> 'select',
					'default'	=> '',
					'el_properties' =>array('class'=>'dropdown2','style'=>$unified_css_style),
					'options'=> array(
									''=>__('Session','etm'),
									'1'=>__('Cookies','etm'),
									'2'=>__('Wp Cache System','etm')
														
								),
					'description'=> __('Using a sophisticated caching sytem on your website can cause the language not to change when going from one page to another.  If this is the case switch to the old session system.','etm'),
	                'save_option'=>true,
					'load_option'=>true
			);				

            $temp[] =(object)array(
				'id'		=> 'trobelshoot_overwrite_translation',
				'label'		=> __('Themes/plugins Translation priority','etm'),
				'type'		=> 'onoff',
				'default'   => false,
                'description'=>__('If not able to translate some plugins or themes, then try turn this option ON. If it doesnt fix the problem please contact support.','etm'),               
				'save_option'=>true,
				'load_option'=>true
				);
			
            $temp[] =(object)array(
				'id'		=> 'trobelshoot_woocommerce_slug',
				'label'		=> __('WooCommerce link problem.','etm'),
				'type'		=> 'onoff',
				'default'   => false,
                'description'=>__('If you are experiencing problems linking to your products in other languages, then try turn this option ON. If it doesnt work please contact support.','etm'),               
				'save_option'=>true,
				'load_option'=>true
				);
	...
	function create_header(){
		...
		jQuery(document).ready(function($){
			...
			jQuery('.pop-onoff-control input[value=""]').each(function(){
				jQuery(this).val('0');
				jQuery(this).parent().find('.pop-onoff button[value="0"]').addClass('checked');
			});
