 ...
 // --------------------------------- CREATE MENU --------------------------------     
    	...
    	if(count($tmp_langs) > 0){ 
            ...
            foreach($tmp_langs as $tmp_lang){
					...
					if(!empty($tmp_lang['fade'])){
					   $tmp_image = '<span style="float: left;line-height: 0;"><img style="box-shadow: none;height:'.$flag_size.'px;" height="'.$flag_size.'" style="filter: alpha(opacity=50);opacity: 0.5;" src="'.$tmp_lang['icon'].'" title="'.$tmp_lang['title'] . '"></span>';
					   $tmp_text = '<span style="float: left;color: #999;margin-left: 5px;line-height: '.$line_height.'px;">'.$tmp_lang['title'] . '</span>';
                    } else {
					   $tmp_image = '<span style="float: left;line-height: 0;"><img style="box-shadow: none;height:'.$flag_size.'px;" height="'.$flag_size.'" src="'.$tmp_lang['icon'].'" title="'.$tmp_lang['title'] . '"></span>';
    ...
    function check_for_translations_permalinks($arrays,$desing_menu_flag_size){
        ...
        if(!empty($arrays)){
			...
			} else if(empty($permalink_array['en'])){
			...
			} else if(empty($permalink_array['us'])){
