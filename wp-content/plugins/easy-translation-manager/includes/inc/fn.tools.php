<?php 
if( !function_exists('etm_query_arg_add') ){
	
	function etm_query_arg_add() {
		$ret = '';
		$args = func_get_args();
		if ( is_array( $args[0] ) ) {
		...
		if(!empty($qs['ETMSTATUSTYPE'])){
			unset($qs['ETMSTATUSTYPE']);
		}	
		
		if(!empty($qs['ETMDELETEPOST'])){
			unset($qs['ETMDELETEPOST']);
		}
		
		if(!empty($qs['ETMSTATUS'])){
			unset($qs['ETMSTATUS']);
		}
		
		if(!empty($qs['ETMDELETEPOSTMETA'])){
			unset($qs['ETMDELETEPOSTMETA']);
		}		
...
function etm_languages_flags($specified = ''){
	...
	$flag_array['sq'] = array('code' => 'sq', 'org_name'=>'Shqip','english_name'=>'Albanian', 'icon'=>'Albania-[size].png','default_locale'=>'sq_AL', 'default_pos'=> 0,'primary_order' =>0);	
	...
	$flag_array['ms'] = array('code' => 'ms', 'org_name'=>'بهاس ملايو','english_name'=>'Bahasa Melayu', 'icon'=>'Malaysia_Bahasa_Melayu-[size].png','default_locale'=>'ms_MY', 'default_pos'=> 0,'primary_order' =>0);
	...
	$flag_array['lg'] = array('code' => 'lg', 'org_name'=>'Oluganda','english_name'=>'Luganda', 'icon'=>'Uganda-[size].png','default_locale'=>'pt_LG', 'default_pos'=> 0,'primary_order' =>0);
	$flag_array['sw'] = array('code' => 'sw', 'org_name'=>'Kiswahili','english_name'=>'Swahili', 'icon'=>'Tanzania-[size].png','default_locale'=>'pt_SW', 'default_pos'=> 0,'primary_order' =>0);
	$flag_array['tn'] = array('code' => 'tn', 'org_name'=>'Setswana','english_name'=>'Setswana', 'icon'=>'Setswana-[size].png','default_locale'=>'tn_BW', 'default_pos'=> 0,'primary_order' =>0);
