    ...
    {  
        foreach($sqldata_tmp as $tmp)
        {
            ...
            if(strlen($tmp->default_string) > 100)
            {
             ...
             if(strlen($tmp->default_string2) > 100)
            {
                $strings2 = substr($tmp->default_string2, 0,100) . '...'; 
            } else {
                $strings2 = $tmp->default_string2;
            } 
                       
            
           $strings = utf8_decode($strings);
           $strings2 = utf8_decode($strings2); 
            
            $return_manual = ' ';
            
            if($tmp->manual_added == 1 or $tmp->mo_tag == 'Variable textdomain'){
	            $return_manual .= '<img style="cursor: pointer; height: 14px; width: 14px; margin-bottom: 1px; margin-right: 5px;" src="'.EASY_TRANSLATION_MANAGER_URL.'images/edit.png" onclick="redigerManualControl(\''.htmlentities($strings).'\',\''.htmlentities($strings2).'\',\''.$tmp->mo_tag.'\',\''.$tmp->id.'\',\''.$etm_tag.'\',\''.$tmp->file.'\')">';
            }
...
function etm_tools_get_post_tags(){
    ...
    if($sort_col == 'date' or $sort_col == 'date'){
	    $sort_col ='name';
    }
