...
class easy_translation_manager_plugin { 
	...
	function easy_translation_manager_plugin( $args=array() ){
		...
		if(empty($this->etm_tools_retrive['trobelshoot_use_sesstion_data'])){
			if(session_id() == '' ){
				session_start();
			}
		}
		...
		if(is_admin()){
			...
			if(!empty($this->etm_tools_retrive['trobelshoot_overwrite_translation'])){
				$this->overwrite_translations();
			}
	...
	function overwrite_translations(){	
		global $wpdb;
	...
	function set_tranlation($to,$data,$type='DEFAULT') {
		
		$data = $data .'BBB'.$type;
		
		if(!empty($this->etm_tools_retrive['trobelshoot_use_sesstion_data'])){
			if($this->etm_tools_retrive['trobelshoot_use_sesstion_data'] == 1){
				setcookie($to,$data,time() + (36000),'/',get_option('siteurl'));
			} else {
				set_transient( $to, $data);
			}
		} else {
			$_SESSION[$to] = $data;
		}
	...
	function get_tranlation($from) {
		$return_data = '';
		if(!empty($this->etm_tools_retrive['trobelshoot_use_sesstion_data'])){
			if($this->etm_tools_retrive['trobelshoot_use_sesstion_data'] == 1){
				$return_data = !empty($_COOKIE[$from]) == true ? $_COOKIE[$from] : '';
			} else {
				$return_data = get_transient($from);
			}
		} else {
			$return_data = !empty($_SESSION[$from]) == true ? $_SESSION[$from] : '';
		}
		
		if(!empty($return_data)){
			$return_data = explode('BBB', $return_data);
			
			if(!empty($return_data[1])){
				$this->etm_return_type = $return_data[1];
			}
    ...
    // check languages
	function check_languages($tmp_overwrite_lang = '',$tmp_ovewrite_front = ''){
		if(!empty($this->selectede_lang) && empty($tmp_overwrite_lang)){
			return '';
		}
		
        $selectede_lang = '';
        $lang_tag_array = '';
        $tmp_return_etm_admin = '';
        $tmp_return_etm_front = '';
        $etm_url_old = '';
        ...
        if(!empty($lang_tag_array)){
            ...
            foreach ($lang_tag_array as $tmp_tag){
                if(isset($_GET[$tmp_tag]) && !empty($_GET[$tmp_tag])){
                	if(is_admin()){
	                	$this->set_tranlation( 'etm_lang_wpadmin', $_GET[$tmp_tag],'GET');
	                	$tmp_return_etm_admin = $_GET[$tmp_tag];
                	} else {
                		$this->set_tranlation( 'etm_lang', $_GET[$tmp_tag],'GET');
                		add_action( 'wp_enqueue_scripts', array(&$this,'woocommerce_script') );
						$tmp_return_etm_front = $_GET[$tmp_tag];
                	}
                    
                    break;
                } else if(isset($_POST[$tmp_tag]) && !empty($_POST[$tmp_tag])){
                	if(is_admin()){
	                	$this->set_tranlation( 'etm_lang_wpadmin', $_POST[$tmp_tag],'POST');
	                	$tmp_return_etm_admin = $_POST[$tmp_tag];
                	} else {
                    	$this->set_tranlation( 'etm_lang', $_POST[$tmp_tag],'POST');
                		add_action( 'wp_enqueue_scripts', array(&$this,'woocommerce_script') );
                		$tmp_return_etm_front = $_POST[$tmp_tag];
                    }
                    break;
                }
            }
            ...
            if(empty($tmp_return_etm_admin) && !empty($tmp_overwrite_lang) && !$tmp_ovewrite_front){
	            $tmp_return_etm_admin = $tmp_overwrite_lang;
	            if(empty($this->etm_tools_retrive['dont_save_overwrite'])){
	            	$this->set_tranlation( 'etm_lang_wpadmin', $tmp_overwrite_lang,'OVERWRITEN');
	            }
            } else if(empty($tmp_return_etm_admin)){
	           $tmp_return_etm_admin = $this->get_tranlation('etm_lang_wpadmin'); 
            }
            
            if(empty($tmp_return_etm_front) && !empty($tmp_overwrite_lang) && $tmp_ovewrite_front){
	            $tmp_return_etm_front = $tmp_overwrite_lang;
				if(empty($this->etm_tools_retrive['dont_save_overwrite'])){
	            	$this->set_tranlation( 'etm_lang', $tmp_overwrite_lang,'OVERWRITEN');
	            }
            } else if(empty($tmp_return_etm_front)){
				$tmp_return_etm_front = $this->get_tranlation('etm_lang');
			}
            
            if(!empty($current_url_multi_domain)){
	            $selectede_lang = $current_url_multi_domain;
	            if(is_admin()){
		            $this->set_tranlation( 'etm_lang_wpadmin', $current_url_multi_domain,'DOMAIN');
	            } else {
		            $this->set_tranlation( 'etm_lang', $current_url_multi_domain,'DOMAIN');
	            }
	            
            } else if((!empty($tmp_return_etm_front) && !is_admin()) or (!empty($tmp_return_etm_front) && !empty($_POST['action']) && strpos($_POST['action'],'woocommerce') !== false )){
                $selectede_lang = $tmp_return_etm_front;
            } else if(!empty($tmp_return_etm_admin) && is_admin()){
                $selectede_lang = $tmp_return_etm_admin;
            } else {
            	...
            	if(!empty($browser_languash)){
            		 ...
            		 if(is_admin()){
            		 	$this->set_tranlation('etm_lang_wpadmin',$lang,'DEFAULT');
            		 } else {
            		 	$this->set_tranlation('etm_lang',$lang,'DEFAULT');
            		 }
            		...
            		if(is_admin()){
            			...
            			if(!empty($default_languashwp)){
            				$this->set_tranlation('etm_lang_wpadmin',$default_languashwp,'DEFAULT');
            			...
            			if(!empty($default_languash)){
            				$this->set_tranlation('etm_lang',$default_languash,'DEFAULT');
    ...
    function aktivate_langushes(){
       ...
       if(empty($this->etm_tools_retrive['trobelshoot_woocommerce_slug']) && !is_admin()){
           add_filter( 'gettext_with_context', array(&$this,'gettext_with_context'), 99999999999, 4);
       }
		...
		if($this->show_lang_status && (!empty($tmp_change_wp_admin) || !is_admin() || (!empty($_POST['action']) && strpos($_POST['action'],'woocommerce') !== false && is_admin() ))){
			...
			if(!empty($etm_mo_files)){
	   		...
	   		//add_filter('the_content',array(&$this,'the_content_wpdb_last_id') ,1);	 
    ...
    function get_pages($maindata) { 
        foreach($maindata as $key => $tmp_data){
            ...
            if($tmp_data->ID > 0){
                ...
                $translatede_body = $this->get_post_meta($tmp_data->ID, 'ect_tran_content_'.$this->selectede_lang, true);
          		$translatede_excerpt = $this->get_post_meta($tmp_data->ID, 'etm_content_excerpts_'.$this->selectede_lang, true);
		        $translatede_title = $this->get_post_meta($tmp_data->ID, 'ect_tran_title_'.$this->selectede_lang, true);
        ...
        foreach($data as $key => $tmp_data){
            ...
            if(!empty($this->etm_tools_retrive['use_permalink']) && $this->etm_tools_retrive['use_permalink']){				
                ...
                if($tmp_data->object == 'page' || $tmp_data->type == 'post'){
                    $translatede_permalink = $this->get_post_meta($tmp_data->object_id, 'ect_tran_permalink_'.$this->selectede_lang, true);
                }
    ...
    function get_metadata($metadata, $object_id = null, $meta_key = null, $single = null){
        if(!empty($this->allow_post_meta) && !is_admin() && $meta_key != '_edit_lock' && $meta_key != '_edit_last' && substr($meta_key,0,4) != 'ect_' && substr($meta_key,0,4) != 'etm_' && substr($meta_key,0,7) != '_yoast_' && substr($meta_key,-3,1) != '_'){
	        
           	$this->allow_post_meta = false;
           	$translations_body = '';
           	if(!empty($object_id) && !empty($meta_key) && !empty($this->selectede_lang)){
				$translations_body = $this->get_post_meta($object_id,$meta_key."_".$this->selectede_lang,true);
				
				//$wpdb->get_var( "SELECT meta_value FROM $wpdb->postmeta where post_id='$object_id' and meta_key='".$meta_key."_".$this->selectede_lang."' LIMIT 1" );	
           	}
	...
	function post_attachment($content,$attachment = null){
        if($attachment->ID > 0){
            $translatede_image_alt = $this->get_post_meta($attachment->ID, 'etm_attachment_image_alt_'.$this->selectede_lang, true);
            $translatede_title = $this->get_post_meta($attachment->ID, 'ect_tran_title_'.$this->selectede_lang, true);
    ...
    function page_link($maindata,$id = '',$test2 = ''){
    	if(!empty($id) && !empty($this->etm_tools_retrive['use_permalink']) && !is_admin()  && !$this->menu_creation){
    		...
    		$translatede_permalink = $this->get_post_meta($id_tmp, 'ect_tran_permalink_'.$this->selectede_lang, true);
	...
	function post_plugin_title($title,$id = ''){
		...
		if($type == 'nav_menu_item'){
			...
			$translatede_title = $this->get_post_meta($id, 'ect_tran_title_'.$this->selectede_lang, true);
		}
    ...
    function post_the_excerpt($content){
		...
		$translatede_excerpt = $this->get_post_meta($post->ID, 'etm_content_excerpts_'.$this->selectede_lang, true);
	...
	function post_plugin_body($content){
        ...
        if($post->ID > 0 ){
            $translatede_body = $this->get_post_meta($post->ID, 'ect_tran_content_'.$this->selectede_lang, true);
		}
	...
	function gettext_with_context( $translated_text, $text, $type,$domain ) {
		
		if($type == 'slug' && $domain == 'woocommerce'){
			return $text;
		}
	...
	function checkpermalink($tmp_name,$tmp_string,$tmp_defualt){
        ...
        if(!empty($id_lang)){
        	if(is_admin()){
	            $this->check_languages($id_lang[1],false);
        	} else {
	            $this->check_languages($id_lang[1],true); 
        	}
        }		
    ...
    function the_content_wpdb_last_id($test1 = ''){
	    global $post,$wpdb;
	    if(!empty($wpdb->last_result) && !empty($wpdb->last_result[0]) && !empty($wpdb->last_result[0]->ID)){
		    $id = $wpdb->last_result[0]->ID;

		    if(!empty($id) && $id > 0 ){
	            $translatede_body = $this->get_post_meta($id, 'ect_tran_content_'.$this->selectede_lang, true);
			}
    ...
    function curPageURL() {
		...
		$pageURL = str_replace('http://localhost/',trailingslashit(get_option('siteurl')), $pageURL);
		$pageURL = str_replace('https://localhost/',trailingslashit(get_option('siteurl')), $pageURL);		
	...
	function woocommerce_cart_item_product($test,$test2){
		if(!empty($test) && !empty($test->id) && !empty($test->post)){
            $translatede_body = $this->get_post_meta($test->id, 'ect_tran_content_'.$this->selectede_lang, true);
      		$translatede_excerpt = $this->get_post_meta($test->id, 'etm_content_excerpts_'.$this->selectede_lang, true);
	        $translatede_title = $this->get_post_meta($test->id, 'ect_tran_title_'.$this->selectede_lang, true);
		...
		if($wp_query->is_search && !empty($wp_query->query['s'])){
			...
			if(!empty($this->selectede_lang)){
				...
				if($defualt_lang_tmp == $this->selectede_lang){
					$tmp_extra = " OR ({$wpdb->posts}.post_title LIKE '%".$wp_query->query['s']."%') OR ({$wpdb->posts}.post_content LIKE '%".$wp_query->query['s']."%')";
				} else {
					$tmp_extra = "";
				}
	
				if(!empty($current_user->ID) and $current_user->ID > 0){
					$where = "AND ((({$wpdb->postmeta}.meta_key = 'ect_tran_title_".$this->selectede_lang."' and {$wpdb->postmeta}.meta_value like '%".$wp_query->query['s']."%') OR ({$wpdb->postmeta}.meta_key = 'ect_tran_content_".$this->selectede_lang."' and {$wpdb->postmeta}.meta_value like '%".$wp_query->query['s']."%')".$tmp_extra.")) AND {$wpdb->posts}.post_type IN ('post', 'page') AND ({$wpdb->posts}.post_status = 'publish' OR ({$wpdb->posts}.post_author = ".$current_user->ID." AND {$wpdb->posts}.post_status = 'private'))";
				} else {
					$where = "AND ((({$wpdb->postmeta}.meta_key = 'ect_tran_title_".$this->selectede_lang."' and {$wpdb->postmeta}.meta_value like '%".$wp_query->query['s']."%') OR ({$wpdb->postmeta}.meta_key = 'ect_tran_content_".$this->selectede_lang."' and {$wpdb->postmeta}.meta_value like '%".$wp_query->query['s']."%')".$tmp_extra.")) AND {$wpdb->posts}.post_type IN ('post', 'page') AND {$wpdb->posts}.post_status = 'publish'";	
				}
			}
