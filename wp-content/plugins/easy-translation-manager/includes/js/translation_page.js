	...
	jQuery(document).ready(function($){
		...
		if(tmp_type == 'meta'){
			if(jQuery('#etm_multi_array_data').length != 0) {
			 	temp_extra = jQuery('#etm_multi_array_data').serializeArray();
			 	temp_genrate_string = '';
			 	if(temp_extra.length > 0){
				 	for(i=0;i<temp_extra.length;i++){
					 	if(typeof(temp_extra[i].value) !== 'undefined' && temp_extra[i].value != ''){
						 	temp_genrate_string += '&'+encodeURIComponent(temp_extra[i].name)+'='+encodeURIComponent(temp_extra[i].value);
					 	}
				 	}
			 	}
			 	temp_extra = '&multidata=true' + temp_genrate_string;
			}
		}
...
function redigerManualControl(tmp_string,tmp_string2,tmp_mo,tmp_id,tmp_tag,tmp_file){ 
	openManualInput(tmp_tag,'',tmp_string,tmp_string2,tmp_mo,tmp_file,tmp_id);

}

// open manaual string	
function openManualInput(tmp_tag,tmp_folder,tmp_string,tmp_string2,tmp_mo,tmp_file,tmp_id){
	jQuery(document).ready(function($){
		aktive_resizer = 1;
		resize_table();
		...
		jQuery('#show_content_box_info').fadeTo('slow', 0.0, function() {
			...
			content_info += '<tr height="50%"><td width="100%" valign="bottom"><textarea style="height:100%;width:100%" id="default_inputtext">'+tmp_string+'</textarea></td></tr>';	
			
			
			content_info += '<tr style="text-align: left; height: 20px; line-height: 10px;"><td width="100%"  valign="bottom">Default Plural (Only if Plural string)</td></tr>';
			content_info += '<tr height="50%"><td width="50%" valign="bottom"><textarea style="height:100%;width:100%" id="default_inputtext_2">'+tmp_string2+'</textarea></td></tr>';			
...
function saveManualInput(tmp_tag,tmp_folder,tmp_defaulttext,tmp_filetext,tmp_motext,tmp_id){
	jQuery(document).ready(function($){
		...
		var default_str_plural = jQuery(tmp_defaulttext+'_2').val();
		...
		jQuery('#show_content_box_info').fadeTo('fast', 0.0, function() {
			...
			jQuery('#show_content_box_info').fadeTo('slow', 1.0, function() {
				...
				jQuery.post(_url,{tag:tmp_tag,folder:tmp_folder,tmp_defaulttext:default_str,tmp_defaultpluraltext:default_str_plural,tmp_filetext:file_str,tmp_motext:mo_str,id:tmp_id,status:tmp_status},function(data){
					jQuery('#show_content_box_info').fadeTo('slow', 0.0, function() {
						if(data.R == 'OK'){ 
...
function setHTMLTinymcePopOpContol_single(tmp_box,boolean_var){
	if(!js_wp_version_is_3_3){
		if(boolean_var){
			if(tmp_box == 1 ){
				switchEditors.go('pp_readonly_content', 'html');	
...
function resize_table(){
		jQuery(document).ready(function($){
			...
			if(button_click_temp_first_time){
				...
				jQuery('#pp_readonly_content-tmce').attr('onclick','setHTMLTinymcePopOpContol_single(1,false);resize_table();');
				jQuery('#pp_readonly_content-html').attr('onclick','setHTMLTinymcePopOpContol_single(1,true);resize_table();');
				jQuery('#pp_translate_content-tmce').attr('onclick','setHTMLTinymcePopOpContol_single(2,false);resize_table();');
				jQuery('#pp_translate_content-html').attr('onclick','setHTMLTinymcePopOpContol_single(2,true);resize_table();');
