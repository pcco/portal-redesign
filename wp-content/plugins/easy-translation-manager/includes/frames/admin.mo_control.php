...
if(!empty($_REQUEST['status']) && $_REQUEST['status']=='update'){
    ...
    $sql = "UPDATE {$wpdb->prefix}etm_plugin_index SET mo_tag='".$_REQUEST['tmp_motext']."',
                												   ...
                												   default_string2='".$_REQUEST['tmp_defaultpluraltext']."',
	...
	if(empty($_REQUEST['tmp_defaultpluraltext'])){
		$_REQUEST['tmp_defaultpluraltext'] = '';
	}
	
		$sqlinsert = "INSERT INTO {$wpdb->prefix}etm_plugin_index (default_string,default_string2,folder_name,file,category_type,mo_tag,create_user,create_ip,manual_added) 
        	  VALUES ('".$_REQUEST['tmp_defaulttext']."','".$_REQUEST['tmp_defaultpluraltext']."','".$_REQUEST['folder']."','".$_REQUEST['tmp_filetext']."','".$_REQUEST['tag']."','".$_REQUEST['tmp_motext']."',".$userdata->ID.",'".$_SERVER['REMOTE_ADDR']."',1)";
