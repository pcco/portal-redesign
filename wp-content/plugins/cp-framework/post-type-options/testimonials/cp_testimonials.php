	...
	class cp_testi_class extends function_library{
				...
				<div style="float:left;" class="op-gap add-music">
					...
					<ul class="recipe_class row-fluid cp_bg_image">
						<li class="panel-title time-start span3">
							<h4><i class="fa fa-music"></i> <?php _e('Icon Name', 'crunchpress'); ?></h4>
						...
						<li class="panel-title border_left time_end span3">
							<h4><i class="fa fa-link"></i> <?php _e('Add URL', 'crunchpress'); ?></h4>
		...
		public function print_testimonial_slider($item_xml){
			...
			jQuery(document).ready(function($) {
				 //Content Slider Home page
					...
					image_array = [
						...
						<?php while ( have_posts() ): the_post();global $post,$post_id;
						$thumbnail_id = get_post_thumbnail_id( $post_id );
						$thumbnail = wp_get_attachment_image_src( $thumbnail_id , 'full');
						?>
						{
							image: '<?php echo $thumbnail[0];?>',
							link_url: '<?php echo get_permalink();?>',
							//link_rel: 'prettyPhoto'
						},
						<?php endwhile;?>
					];
					$('#testi_slider-<?php echo $counter;?>').content_slider({ // bind plugin to div id="slider1"
				...
				$('.circle_slider_nav_left').children().remove();
				$('.circle_slider_nav_right').children().remove();
			...
			<article class="testimonials">        	
				...
				<div class="content_slider_wrapper" id="testi_slider-<?php echo $counter;?>">				
					<?php 
					...
					?>
					<!--LIST ITEM START-->
					<div class="circle_slider_text_wrapper" id="sw<?php echo $testi_monial_slider;?>" style="display: none;">
						<div class="content_slider_text_block_wrap">
							<span><?php echo substr(get_the_content(),0,150);?></span>
							<h3><?php echo get_the_title();?></h3>
							<?php if($designation_text <> ''){ ?><strong class="title"><?php echo $designation_text;?></strong><?php }?>
							<?php
							//Fetching All Tracks from Database
							$track_name_xml = get_post_meta($post->ID, 'add_icon_xml', true);
							$track_url_xml = get_post_meta($post->ID, 'add_project_field_xml', true);
							...
							if($track_name_xml <> '' || $track_url_xml <> ''){
								...
								for($i=0;$i<$nofields;$i++) { 
									$counter++;?>
										<a href="<?php echo $children_title->item($i)->nodeValue;?>"><i class="<?php echo $children_name->item($i)->nodeValue;?>"></i></a>
									<?php
								}
							} ?>
						</div>
