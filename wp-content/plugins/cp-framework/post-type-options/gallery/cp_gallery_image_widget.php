...
?>
  ...
  <label for="<?php echo $this->get_field_id('select_gallery'); ?>">
	  <?php _e('Select Gallery:','crunchpress');?>
	  <select id="<?php echo $this->get_field_id('select_gallery'); ?>" name="<?php echo $this->get_field_name('select_gallery'); ?>" class="widefat">
		<?php
        global $wpdb,$post;
  ...
  <label for="<?php echo $this->get_field_id('nofimages'); ?>">
	 <?php _e('Number of Images to Show:','crunchpress');?> 
		...
		if (!empty($title))
			...
			<ul class="flickr-list">
				...
				$counter_limit++;
					...
					echo '<li><div class="thumb"><a data-rel="prettyPhoto[]" href="' . $image_full[0] . '"  title=""><img src="' . $image_thumb[0] . '" alt="' . $alt_text . '" /></a></div><p>'.$title.'</p></li>';
				}?>				
			</ul>
			<?php
		}	
