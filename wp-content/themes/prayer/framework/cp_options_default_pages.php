...
function default_pages_settings(){
  ...
  <ul id="wp_t_o_right_menu">
				<li id="active_tab" class="default_pages"><?php _e('Default Pages Settings', 'crunchpress'); ?></li>
			</ul>
        </div>
      </div>
      <!--sidebar end --> 
	  ...
	  <?php //echo top_navigation_html(); ?>
        <form id="options-panel-form" name="cp-panel-form">
          <div class="panel-elements" id="panel-elements">
			...
			<h3> <?php _e('Category Pages, Search, Archives, Taxonomy, Tags.','crunchpress');?> </h3>
			...
			<ul class="default_excerpt recipe_class row-fluid">
				<li class="panel-input span8">
					<span class="panel-title">
						<h3 for="default_excerpt" > <?php _e('Default Excerpt', 'crunchpress'); ?> </h3>
					</span>	
					<input type="text" name="default_excerpt" id="default_excerpt" value="<?php if($default_excerpt <> ''){echo $default_excerpt;};?>" />
				</li>
				<li class="description span4"><p><?php _e('Please Paste Your Default Excerpt(Number of words).','crunchpress');?></p></li>
			</ul>
